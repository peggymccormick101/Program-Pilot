...
def generate_roadmap_options(features: list[dict], capacity: dict) -> dict:
    ...
    return _run_structured(
        system=(
            ...
            "confidence, capacity, and parallel work. Each Feature's "
            "feature_dependencies field lists the FID(s) (feature_id) of "
            "other Features it depends on -- cross-reference those FIDs "
            "against every Feature's own feature_id to resolve the actual "
            "dependency, and never schedule a Feature into an earlier "
            "release than any Feature it depends on (the same release is "
            "fine). For each option, sequence Features by release, show "
            "capacity use, sequencing rationale (naming any dependency "
            "that drove the sequencing), key risks, and deferred "
